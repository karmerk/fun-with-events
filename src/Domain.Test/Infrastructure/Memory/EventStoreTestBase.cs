...
namespace Domain.Test.Infrastructure.Memory
{
    public abstract class EventStoreTestBase
    {
        ...
        {
            await EventStore.AppendAsync("one", GenerateTestEvents(0).Take(10));
            await EventStore.AppendAsync("two", GenerateTestEvents(0).Take(15));
            await EventStore.AppendAsync("three", GenerateTestEvents(0).Take(20));
        ...
        [TestMethod]
        [DataRow(10, 5, 5, DisplayName = "Only 5 events returned altho 10 events exists")]
        [DataRow(5, 10, 5, DisplayName ="All 5 events are returned when count is 10")]
        public async Task GetAsync_CountLimitsResult(int numberOfEventsToInsert, int countToRead, int expectedEventsRead)
        {
            var name = nameof(GetAsync_CountLimitsResult);

            await EventStore.AppendAsync(name, GenerateTestEvents().Take(numberOfEventsToInsert));
        ...
        public async Task GetAsync_BeginSkipsPriorEvents(int numberOfEventsToInsert, int begin, int expectedEventsRead)
        {
            var name = nameof(GetAsync_CountLimitsResult);
        ...
        // TODO test GetAsync begin combined with count

        // TODO test GetBackwards
    }
}
